...
const Hero: React.FC = () => {
  // 사용자가 깃허브에 올릴 사진 파일명을 'profile.png'로 설정했습니다.
  const instructorImageUrl = "/profile.png"; 
      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center w-full relative z-10">
        ...
        <div className="order-2 md:order-1 space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-1000">
          <div className="space-y-4">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-black leading-[1.15] text-slate-900 tracking-tight">
              <span className="block mb-2 md:whitespace-nowrap">승패를 가르는 1%,</span>
              <span className="text-gradient block md:whitespace-nowrap">멘탈에 답이 있다.</span>
            </h1>
          </div>
        ...
        <div className="order-1 md:order-2 flex justify-center relative animate-in fade-in zoom-in duration-1000 delay-200">
          <div className="relative group w-full max-w-[420px]">
            ...
            <div className="glass-card p-3 rounded-[3rem] relative z-10 w-full shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] overflow-hidden transition-all duration-700 group-hover:scale-[1.02]">
              <div className="relative overflow-hidden rounded-[2.5rem] w-full aspect-[3/4] bg-slate-200">
                <img 
                  src={instructorImageUrl}
                  alt="장윤정 멘탈코치" 
                  className="w-full h-full object-cover object-top hover:scale-110 transition-transform duration-[2000ms] ease-out"
                  onError={(e) => {
                    // 사진이 없을 경우 보여줄 임시 이미지입니다.
              ...
              <div className="absolute bottom-8 left-8 right-8 bg-white/95 backdrop-blur-2xl border border-white/50 p-6 rounded-[2rem] shadow-2xl transform group-hover:translate-y-[-10px] transition-transform duration-500">
                <div className="flex items-center gap-2 mb-2">
                  <div className="h-1 w-8 bg-orange-500 rounded-full"></div>
                  <p className="text-orange-600 text-[10px] font-black uppercase tracking-[0.3em]">Mental Coach Specialist</p>
                </div>
                <div className="flex items-baseline gap-2">
                  <h2 className="text-slate-900 font-black text-2xl">장윤정</h2>
                  <span className="text-slate-400 font-light text-lg tracking-tight">Jang Yun Jeong</span>
                </div>
                <div className="mt-2 flex flex-col text-slate-500 text-[11px] gap-1 font-semibold border-t border-slate-100 pt-3">
                  <p>• 사회복지학 박사 (Ph.D)</p>
                  <p>• ASTAF 국제 세팍타크로 심판</p>
                  <p>• (주)제이에듀 대표이사</p>
                </div>
              </div>
            </div>
